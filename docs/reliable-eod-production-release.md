...
The first live transfer exposed two D1 server SQL-splitting differences: a bare `CASE ... END;` inside a trigger was mistaken for the trigger boundary, and the trailing accounting comment became an empty second statement. The follow-up preserves trigger behavior with parentheses and removes only the transport suffix of exact reviewed DDL; accounting still uses the original statements. All nine corrected history guards installed successfully on live D1 (zero reads, nine writes), with its fence still open. The failed run was aborted and its newly created, verified-empty target removed; source and history data were preserved. The corrected migration starts with its own pinned revision. All 66 focused SQL/adapter tests, Worker/runner typechecks and the Worker build passed for this follow-up.

An idempotent restart also exposed SQLite retaining the older reviewed-DDL accounting comment in the history fence's stored schema. The starter now recognizes that exact suffix while preserving schema tokens and quoted values. Real SQLite regressions cover both stored formats and reject changed constraints, types and defaults; the corrected matcher also passed against the actual production history definition. The second attempt was stopped before authorization, and only its independently verified empty target was removed. Existing source/history data remain preserved.

The next live run reached target initialization and exposed a D1 metadata mismatch: a successful one-row archive-pointer update reported changes=2 because of the storage guard, while a rejected conditional upsert reported changes=1 from its BEFORE trigger alone. Direct RETURNING results correctly distinguished the two live cases. Archive promotion and affected repair/refresh ownership checks now verify returned row identities; maintenance and daily-bar counts use actual affected rows while quota accounting retains all billed writes. The SQLite bridge models trigger-inclusive D1 changes. The interrupted target contained no business rows; that was checked across every reviewed table before abort and removal. The original source was released intact, and the newly archived block was retained.

Both EOD and compatibility bar writers also filter unchanged observations atomically before INSERT, so the five-session overlap does not fire write guards for identical prices. The updated EOD writer passed a live existing-price replay with four reads, zero writes, no returned changed rows and an unchanged revision clock. Focused regressions retain genuine price corrections and verify feature invalidation starts only at an actually changed observation.

The final trigger-contract follow-up passed the complete Worker suite: 188 files / 1,450 tests. Worker and runner typechecks and the Worker build also passed. Web source is unchanged since its previously verified 110 tests, typecheck and production build.
