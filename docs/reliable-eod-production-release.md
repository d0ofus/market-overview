...
Three consecutive real trading sessions meeting the actual-close-plus-two-hours deadline and agreed budgets are required before retiring the legacy implementation, under the policy updated on 11 September 2026. Finalized whole-day usage is required for those trading dates; weekend and holiday buckets do not gate retirement. A deployed release, healthy database connection or successful dispatch does not establish those conditions. Follow the [rollout runbook](reliable-eod-rollout.md) for recovery, activation and archive-compatible rollback.
...
Daily operating evidence now records actual public activation, first complete publication timing and finalized UTC account usage. Missing trading-date evidence remains pending. The current retirement policy requires three consecutive real trading sessions and excludes weekend/holiday buckets from that gate; private bootstrap results cannot count toward the streak. The admin EOD panel shows these checks and the measured storage forecast separately.
...
Inspect the admin EOD panel first: pipeline mode, failed stage, next retry, expected/displayed sessions, storage forecast and monitoring streak. Then inspect all Overview sections and five Breadth universes for dates, coverage, null fields and explicit missing-history reasons. Existing dated public data remains expected until final acceptance and verified binding cutover. The workflow names are **EOD storage migration**, **EOD market data** and **EOD operating evidence**. A successful dispatch or deployment is not evidence that their data stages completed.
...
At 22:36 UTC on 9 September, account analytics reported **5,049,746 reads / 77,772 writes**. No further live D1 acceptance work was attempted against that exhausted allowance. The source remained unfrozen, the capture remained partial, and no replacement database or public cutover had been created. This release changes operator automation and its tests; the deployed application bundle is unchanged. Migration and Worker binding deployments remain part of the guarded recovery sequence after the reset. The subsequent retirement-policy update requires three real trading sessions of monitored operation with whole-day usage for those trading dates and no weekend/holiday bucket requirement.

# 2026-09-11 three-session policy and daily Admin status

At the user's direction, the retirement gate now requires three consecutive trading sessions with finalized usage, without a weekend check. A shared finalizable-session cutoff prevents new unsettled usage from continually moving the gate. Known newer delivery failures still block approval. Previous ten-session caches cannot satisfy the new versioned policy. Price correctness, publication coverage, storage headroom and daily quotas retain their existing limits.

The prominent `/admin` EOD recovery card separates recovery/cutover, verified production configuration and the three-session gate. It displays the last controller report, its age, the blocker, the next retry and whether the recovery computer is needed. Reports reuse the existing Ops evidence table and an authenticated read endpoint; this change requires a Worker deployment and the web deployment, with no database migration. The final configuration milestone requires the live-verifying recorder described in [the recovery runbook](reliable-eod-local-recovery.md).
