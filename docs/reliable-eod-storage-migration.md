...
The source is never deleted by this runner. A quota failure records the next UTC reset; network/server failures and a 65-minute time slice resume from checkpoints. Verification failures pause for investigation. Copy writes are idempotent. Up to 16 archive blocks may be replayed after interruption; duplicate blocks are verified and reused. Non-price copy and independent table verification use 250-row primary-key pages. Copy readback checks every row and admits one extra destination key to detect contamination; pages above 1.8 MB subdivide before the D1 value limit. Existing 100-row checkpoints retain their captured identity, final key and accumulated hash, then continue with the larger pages. Large tables publish verified row progress at completed-page boundaries approximately every 30 seconds, and on table completion.
...
If a code fix is needed after population preparation but before sizing approval or any consumer/bootstrap checkpoint, `worker/scripts/withdraw-storage-population-plan.ts` can withdraw the exact selected plan before the normal executor transition. Supply `EOD_STORAGE_WITHDRAW_PLAN_HASH` and the previous execution revision alongside the existing database identities. It requires a clean, pushed checkout, no writers, the sizing-required state and matching frozen source/target/history captures. It preserves the immutable plan, any sizing evidence and every copy checkpoint; only the mutable selection is removed, with a checksummed audit record. The new executor must prepare and validate a fresh plan. This path cannot transfer prior consumer, bootstrap or publication acceptance to new code.
...
Proceed to deployment only if approval succeeds. Keep the original `EOD_STORAGE_CODE_REVISION` and `EOD_STORAGE_SOURCE_DATABASE_ID` GitHub variables unchanged; the migration continues to refer to its original accepted code and source. The canonical market variable and runner mode must already match the activated target. Daily and monitor workflows check out `EOD_PRODUCTION_CODE_REVISION` when configured, so a new `main` commit does not itself change their approved runtime. Keep that pin on the activated revision until the recording command verifies and advances it to the configuration revision.
...
Finally, the recording command independently verifies the new sole serving Worker version, the measured 90-session approval and canonical checked-in/GitHub settings. After all other checks pass, it sets GitHub's `EOD_PRODUCTION_CODE_REVISION` to the new revision and `EOD_ARCHIVE_PRUNE_ENABLED` to `true`, re-reads their actual values and the serving version, then writes `recovery:production-configuration`. Its output must say `recorded`; a quota failure leaves that milestone pending and can be retried. Public activation keeps its original SHA/date while the configuration record identifies the new deployment SHA/version.
...
The measured retention model covers both SIP and Yahoo, with 260 or 90 recent sessions and a fixed future exchange-session horizon. A forecast does not renew itself merely because maintenance ran. Expiry, population growth and unexpectedly large retained windows require new measured evidence. The admin panel exposes this separately from database connectivity. [Monitoring](reliable-eod-monitoring.md) credits public delivery only after actual activation and keeps finalized account-wide UTC usage, including weekends, as operational history. No elapsed observation period or weekend check is required for recovery completion; current publication, quota, capacity and correctness checks still apply. A late initial recovery remains dated as late after current delivery recovers.
...
Population drift during a multi-day migration remains a separate acceptance boundary. After whole-copy verification, the private target prepares the latest memberships and shared catalog and records an immutable population plan with a fresh target capture and the original verified history capture. The `storage-population-sizing-required` pause invokes the local controller's measured replan: export `population-inputs`, write a separate plan-hash ticker artifact, run the full-population model against the original captured snapshots, and call `approve-population`. The original source date, ticker file, hashes and copy checkpoints remain unchanged. Approval resumes the existing migration for new full-population reader parity and private bootstrap; it does not grant cutover. Final publication growth and sizing also use that exact plan population. Further unexplained drift still fails closed. Do not trim current constituents to an older population, substitute a universe, or redate old source evidence to force a match. The copied source and archives remain preserved throughout.

A price-input correction in an already completed private session queues that owned daily run again; unchanged calculations can retain their revision-matched checkpoints, and accepted publications remain available while corrected revisions are built. A later exchange session with the same shared population gets a separately dated input plan and bootstrap owner, retaining the actual earlier storage measurement and parity dates. If the shared population changes after bootstrap has written corrected prices, recovery pauses with `storage-population-live-recapture-required`. This requires a fresh capture and independent full-history reference with full-population sizing and reader parity before continuing. The original frozen source cannot serve as a newly dated expected result after those corrections. This condition stays visible in migration/controller status with no automatic retry loop or public cutover.
