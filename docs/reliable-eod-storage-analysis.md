...
| `--fallback-ticker-reserve` | Omitted | Default models the enforced Yahoo archive capacity of 1,000 distinct securities, including old/staged immutable blocks. Supplying a number requests a diagnostic legacy dual-hot model; reducing that number cannot pass the production gate. |
...
4. Record captured-history uncompressed JSON, gzip and base64 payload bytes and its physical size. Then measure an additional Yahoo fallback fixture in the same disposable archive using the actual codec and history schema. It covers 320 observations per reserved security, including forty future weekday slots, with varied full-width OHLC and volume values. Existing immutable revisions and identities outside today's shared population remain included. Reserve twice this combined physical size plus 4 MiB for another complete revision and transient capacity. This finite forecast does not cover indefinitely accumulating revisions, future years or missing deep backfill.
...
6. Populate disposable 260+10 and 90+10 SIP capacity models for every frozen shared ticker, using the source's actual schema and indexes. Yahoo EOD prices are archived directly, so the default model does not duplicate them into a second hot table window. Preserve other feed/non-shared seed rows and existing auxiliary data; include compact SIP and Yahoo revision/repair state for the whole population. Use full-width numeric fields and populated timestamps. These synthetic capacity fixtures are never deployment input or financial observations; their dates are storage slots, not exchange-calendar evidence.
...
The `archive-only-bounded-v1` layout preserves the entire primary ingestion universe. Its secondary provider has a separate enforced limit of 1,000 archived security identities, counted across all retained immutable blocks. A new identity beyond that capacity produces visible unavailable fallback coverage; it does not displace old history or disappear from membership denominators. The daily Yahoo request limit remains separate. Source histories already beyond this storage capacity fail sizing and require a reviewed capacity change. The 350 MB database gate and 64 MB relocation planning reserve remain unchanged. Final acceptance still requires measured real publication growth and actual D1 sizes.
