...
Before retiring legacy writers/storage, store version-2 Ops evidence ID `retirement`: exact `policyVersion:"three-trading-sessions-v1",codeRevision,methodologyVersion` and three `sessions` entries containing `sessionDate,runId,deadlineAt,publishedAt,scopes,measurements,limits`. `assertEodRetirement` verifies the latest three consecutive cached exchange sessions at or before the shared usage-finalization cutoff (the earlier of the expected session and UTC today minus two days), active daily run references, six accepted publications by each real deadline, finalized whole-day quota for those exchange dates and the same budget/Free limits. A newer day's unsettled usage cannot move this window forward indefinitely; known newer delivery failures still prevent approval. Weekend and holiday buckets remain operational telemetry and do not block retirement. Old policy versions, missing finalizable trading evidence or stale/missing proof block retirement; cutover alone does not authorize it.
