...
Initial activation requires a measured full-catalog shadow run and validated cutover evidence, not ten shadow sessions. Retain the commit, session calendar/actual close, configuration/membership versions, six page scope IDs, the run progress `catalogPublicationId` reference, the completed input-clock watermark, row/metric coverage, provider errors, publication completion time, account quota samples and capacity measurements. Ten consecutive completed exchange sessions are required later, before legacy retirement.
...
Before retiring legacy writers/storage, store version-1 Ops evidence ID `retirement`: exact `codeRevision,methodologyVersion` and ten `sessions` entries containing `sessionDate,runId,deadlineAt,publishedAt,scopes,measurements,limits`. `assertEodRetirement` verifies the latest ten consecutive cached exchange sessions, completed active daily runs, six accepted publications by each real deadline, recorded quota and the same budget/Free limits. Missing sessions or stale/missing proof block retirement; cutover alone does not authorize it.
