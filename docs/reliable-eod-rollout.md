...
| --- | --- |
| `version`, `codeRevision`, `methodologyVersion` | `1`, exact 40-character verified checkout revision supplied as `EOD_CODE_REVISION`, current `EOD_METRICS_VERSION` |
...
| `limits` | Declared `httpCpuMs,coordinatorCpuMs,queriesPerInvocation,queryDurationMs`; cannot exceed the explicitly selected budget profile: Free 10ms/10ms/50/30,000ms, Paid 1,000ms/1,000ms/300/30,000ms; SQL duration must be strictly below its limit |
...
Measured usage plus outstanding reservations must stay within the explicitly selected `EOD_BUDGET_PROFILE`, which defaults to `free`. Free allows EOD use up to 2.5m reads/50k writes and account use up to 4.5m/90k per UTC day. The current Paid application ceilings are EOD 1b reads/8m writes and account 1.5b/10m per UTC day, plus an account-wide rolling 31-day ceiling of 20b reads/35m writes. These are admission ceilings, not measured usage or permission to expand the storage layout; the 350MB database targets and approved 90-session retention remain unchanged. Initial validation also checks recorded counters against the selected ceilings and verifies the completed global input watermark, including Yahoo revisions; counters can rise after a measurement. Changed inputs require a fresh validated completion before initial approval. A successful check persists a checksummed durable approval under `active:<EOD_CODE_REVISION>`, including the original proof/hash and approval timestamp. Later runs of that exact revision use this approval without expiring or redating the old measurement; live quota/capacity admission still applies on every run. A new code revision requires a new matching measured proof or the strictly validated configuration-only transition. The guard validates proof structure/reference consistency, not the truth of independently collected CPU telemetry. Initial CPU and full-population parity evidence remain independently collected; the ongoing history-capacity sampler described below does not manufacture those approvals. See [Workers limits](https://developers.cloudflare.com/workers/platform/limits/) and [D1 limits](https://developers.cloudflare.com/d1/platform/limits/).
...
The Node runner verifies its actual checked-out Git revision and supplies it as `EOD_CODE_REVISION`. Production workflows check out the approved `EOD_PRODUCTION_CODE_REVISION` pin, which can differ from the initiating workflow's `GITHUB_SHA`; they use `github.sha` only when no production pin exists. After validating the independent initial proof, the legacy maintenance path records a checksummed durable approval at `history-approval:<SHA>`; the storage migration path uses its measured `history-storage-approval:<SHA>` and immutable proof. These approvals retain the original reader parity timestamp. A new code revision needs a matching independent proof or the strictly validated configuration-only reference; neither path redates old measurements. Automatic capacity sampling never claims to have rerun consumer parity or the initial full-population validation.
