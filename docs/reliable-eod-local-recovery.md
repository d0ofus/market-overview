...
Publication evidence is stored in separate ignored directories by code revision, session and actual sample hash. Retrying a different session or correction cannot silently reuse another payload's growth artifact. Historical runtime collection re-fetches authenticated Cloudflare logs at acceptance. The optional live-tail path instead loads the immutable receipt written to Ops by the pinned authenticated collector, checks all five durable samples, and rechecks current publications, input clock and the actual 100% serving candidate version. It does not describe live logs as replayable historical records or trust a manually edited local JSON file. Actual 100% serving production bindings and GitHub canonical settings are checked before marking the migration complete. See [the runtime collection protocol](reliable-eod-runtime-collection.md).
