...
| 5. Recovery and cutover controls | [Rollout gates](../worker/src/eod-rollout-service.ts), [capacity measurement](../worker/src/eod-history-capacity.ts), [history jobs](../worker/src/eod-history-runner.ts), [operator runbook](reliable-eod-rollout.md) | Same-session correction recovery, independent immutable publication pointers, retained per-session revisions, capacity/parity proof, quota-aware maintenance and archive-compatible rollback. Admin records verified production cutover and final configuration separately; current health replaces an elapsed observation requirement. |
