...
Before active ownership, collect a full-universe shadow run with actual D1 billed reads/writes, Worker CPU/query measurements, provider/session validation and consumer parity. The guard requires a matching code revision and recorded publication references; it cannot manufacture live measurements. Before retiring the replaced implementation, observe **10 consecutive real trading sessions** meeting the actual-close-plus-two-hours deadline and agreed budgets. No such observation period is claimed by this implementation.
