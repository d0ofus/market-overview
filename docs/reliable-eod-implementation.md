...
Before active ownership, collect a full-universe shadow run with actual D1 billed reads/writes, Worker CPU/query measurements, provider/session validation and consumer parity. The guard requires a matching code revision and recorded publication references; it cannot manufacture live measurements. Before retiring the replaced implementation, observe **3 consecutive real trading sessions** meeting the actual-close-plus-two-hours deadline and agreed budgets, including finalized whole-day usage for those trading dates. Weekend and holiday buckets remain operational telemetry and do not gate retirement. This updated policy replaces the earlier ten-session requirement; no completed observation period is claimed by this implementation.
