...
Yahoo identifies the VIX index with the `America/Chicago` exchange timezone; the other reviewed index mappings retain their own explicit identity checks. All Overview calculations still use the shared US equity-session grid. A full-window recovery check found all 260 required VIX sessions plus two observations on equity-market holidays. The VIX-specific selector excludes and records off-grid observations before storage or calculation. It does not move their dates, add holidays to the calendar, fill missing anchors, or relax another instrument's validation. A valid VIX window can therefore remain usable even when Yahoo also returns extra holiday observations.

The issuer changed RSHO to WELD effective June 22, 2026. The configured RSHO key remains a historical alias for the same fund, with WELD identified in its displayed name and source explanation. For sessions from that date, a separate Alpaca request uses RSHO's last pre-change session, June 18, as its symbol-identity `asof` date. Alpaca documents that this returns the successor's observations under the historical alias; other symbols retain their own requested-session identity. Yahoo uses WELD with the issuer's exact current fund name, ETF/USD/New York metadata and the unchanged two-session SIP price-basis comparison. Earlier sessions retain their original symbol identity. Sources: [Tema's dated ticker-change notice](https://temaetfs.com/rsho-landing-page), [Alpaca historical-bar symbol mapping](https://docs.alpaca.markets/us/reference/stockbars).
