...
The authorized production rollout applied this index on 9 September 2026 at 00:00:10 UTC after the exhausted daily allowance reset. D1 reported 20,297 reads and 10,120 writes. The production query plan now uses `SEARCH symbols USING INDEX idx_symbols_upper_ticker`. Subsequent symbol changes maintain the index. Continue comparing account-wide usage and indexed-query insights; remaining earnings-row scans and other workflows still consume shared quota. See the [production release record](reliable-eod-production-release.md). A clean EOD counter alone does not prove account headroom.
