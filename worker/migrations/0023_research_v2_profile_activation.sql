...
SET model_family = 'claude-3-haiku-20240307',
    schema_version = 'v2',
    template_text = 'Extract a deep, evidence-grounded research card. Preserve evidence IDs, separate facts from inferences, make peer comparison optional but first-class, and return strict JSON only.',
    template_json = '{"responseShape":"research-card-v2","maxEvidenceItems":20}'
WHERE id = 'prompt-haiku-extract-v2';

UPDATE prompt_versions
SET model_family = 'claude-3-5-sonnet-20241022',
    schema_version = 'v2',
    template_text = 'Reconcile deterministic factor cards into an auditable final ranking. Use the factor cards as the canonical base signal, make only bounded adjustments, and return strict JSON only.',
    template_json = '{"responseShape":"run-ranking-v2","maxTickers":25}'
WHERE id = 'prompt-sonnet-rank-v2';

UPDATE prompt_versions
SET model_family = 'claude-3-5-sonnet-20241022',
    schema_version = 'v2',
    template_text = 'Write a true PM-style research synthesis from the structured card plus evidence packets. Prioritize priced-in analysis, underappreciated evidence, leadership, invalidation, and peer context when credible.',
    template_json = '{"responseShape":"ticker-deep-dive-v2","maxEvidenceItems":24}'
...
SET prompt_version_id_haiku = 'prompt-haiku-extract-v2',
    prompt_version_id_sonnet_rank = 'prompt-sonnet-rank-v2',
    prompt_version_id_sonnet_deep_dive = 'prompt-sonnet-deep-dive-v2',
    rubric_version_id = 'rubric-swing-v2',
    search_template_version_id = 'search-template-swing-v2',
    settings_json = '{
      "lookbackDays": 14,
      "includeMacroContext": true,
      "maxTickerQueries": 7,
