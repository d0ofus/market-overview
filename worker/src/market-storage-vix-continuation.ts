...
import { storageIndexLoaderContinuationKey } from "./market-storage-history-index-continuation";
...
export const storageVixContinuationKey = (id: string, revision: string) => `storage-vix-continuation:${id}:${revision}`;
...
function fail(reason: string): never { throw new Error(`storage-vix-continuation-${reason}`); }
...
/** Authenticate the correction and its actual R15 predecessor separately. The
 * existing R15 loader then authenticates the original R14 index amendment.
 * Historical plans, captures and dates are never relabelled as new measurements. */
export async function loadStorageVixContinuation(ops: D1Database, run: StorageMigrationRun, plan: StoragePopulationPlan): Promise<{
...
export async function approveStorageVixContinuation(input: { ops: D1Database; source: D1Database; target: D1Database; history: D1Database;
  ...
  if (run.execution_revision === input.codeRevision) {
    ...
    const continuation = await verified(parse<Continuation>(await read(ops, storageVixContinuationKey(run.id, input.codeRevision))));
  ...
  if (run.status !== "queued" || run.stage !== "bootstrap" || run.error_code !== null || run.freeze_authorized !== 1
    || !run.freeze_evidence_hash || run.dispatch_token !== null || run.lease_token !== null || run.lease_until !== null
    || !run.next_attempt_at || !Number.isFinite(Date.parse(run.next_attempt_at)) || Date.parse(run.next_attempt_at) > now.getTime()) fail("released-bootstrap-slice-required");
  ...
  if (!eod || eod.status !== "retrying" || eod.mode !== "active" || eod.stage !== "prices" || eod.purpose !== "daily"
    || eod.error_code !== "runner-error" || eod.error_message !== "storage-run-time-slice-complete"
    || eod.lease_token !== null || eod.lease_until !== null || eod.dispatch_token !== null || typeof eod.next_attempt_at !== "string"
    || !Number.isFinite(Date.parse(eod.next_attempt_at)) || Date.parse(eod.next_attempt_at) > now.getTime()
    || !await same(parse(eod.input_json), previous.inputs) || !progress || progress.total !== total
    || !Number.isSafeInteger(progress.chunk) || Number(progress.chunk) < 0 || Number(progress.chunk) >= total
    || !Number.isSafeInteger(progress.symbols) || Number(progress.symbols) < 0 || Number(progress.symbols) > previous.tickers.length) fail("planned-slice-required");
  ...
  const vixIndex = previous.inputs.tickers.indexOf("VIX");
  if (vixIndex < 0 || previous.inputs.tickers.lastIndexOf("VIX") !== vixIndex) fail("vix-not-in-plan");
  const vixKey = `features:${Math.floor(vixIndex / 25)}`, vixRow = features.find(row => row.chunk_key === vixKey);
  const vixCheckpoint: Continuation["vixCheckpoint"] = { key: vixKey, status: "missing", hash: null };
  if (vixRow) {
    const encoded = object(parse(vixRow.payload_json));
    if (!encoded) fail("vix-checkpoint-invalid");
  ...
  const publicationSnapshot = async () => {
    ...
    const values: Continuation["publications"] = [];
  ...
  const records = [{ id: storageVixContinuationKey(run.id, input.codeRevision), value: continuation },
  ...
  const queries: Array<{ sql: string; params: unknown[] }> = [{ sql: `SELECT CASE WHEN EXISTS(SELECT 1 FROM market_storage_migrations WHERE id=? AND updated_at=?
      AND status='queued' AND stage='bootstrap' AND error_code IS NULL AND COALESCE(execution_revision,code_revision)=? AND execution_evidence_hash=?
    ...
    AND NOT EXISTS(SELECT 1 FROM json_each(?) expected LEFT JOIN eod_checkpoints actual ON actual.run_id=? AND actual.chunk_key=json_extract(expected.value,'$.chunk_key')
      WHERE actual.chunk_key IS NULL OR actual.input_hash<>json_extract(expected.value,'$.input_hash') OR actual.payload_json<>json_extract(expected.value,'$.payload_json') OR actual.updated_at<>json_extract(expected.value,'$.updated_at'))
