...
function buildSectionSpec(promptConfig: ResearchLabPromptConfigRecord): string {
  ...
  const sections = [
    ...
    "catalysts[{title,summary,direction:positive|negative|mixed,timeframe,evidenceIds[]}]",
  ...
  const response = await callResearchLabSonnetJson<ResearchLabSynthesis>(env, {
    promptConfig: {
      ...
      systemPrompt: [
        ...
        "For each catalyst, direction must be exactly one of: positive, negative, or mixed.",
