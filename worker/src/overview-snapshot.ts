...
export async function isOverviewSnapshotStale(env: Env, configId = DEFAULT_CONFIG_ID): Promise<boolean> {
  ...
  if (latest.asOfDate > latestUsMarketSessionAsOfDate(new Date())) return true;
  ...
  if (uniqueTickers.length > 0) {
    ...
    ).bind(marketDataFeed(env), ...uniqueTickers, latest.asOfDate).all<{ ticker: string; date: string; c: number }>();
