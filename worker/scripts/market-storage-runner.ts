...
import { loadStoragePopulationPlan, loadStorageValidationPlan, approveStoragePopulationSizing } from "../src/market-storage-population-plan";
...
async function main():Promise<void> {
  ...
  try {
    ...
    if (command==="population-inputs") {
      await assertOriginalCapture();
      const plan=await loadStoragePopulationPlan(meteredOps,existing);
      if (!plan) throw new Error("storage-population-plan-required");
      writeFileSync(required("EOD_STORAGE_POPULATION_PLAN_PATH"),JSON.stringify({version:1,planHash:plan.planHash,
        migrationId:id,codeRevision,sessionDate:existing.session_date,bootstrapSessionDate:plan.sessionDate,
        tickers:plan.tickers,tickerHash:await storageHash(plan.tickers)}));
      console.log(JSON.stringify({id,status:"population-inputs-exported",planHash:plan.planHash,
        tickerCount:plan.tickers.length,bootstrapSessionDate:plan.sessionDate}));return;
    }
    if (command==="approve-population") {
      await assertOriginalCapture();
      if (existing.status!=="awaiting-evidence" && existing.status!=="queued") throw new Error("storage-population-approval-state-conflict");
      const sizingHash=await approveStoragePopulationSizing(meteredOps,existing,{analysis:file("EOD_STORAGE_ANALYSIS_PATH"),accountId,
        snapshotSource:file("EOD_STORAGE_SNAPSHOT_IDENTITY_PATH") as {accountId:string;sourceDatabaseId:string;runId:string}});
      if (existing.status==="awaiting-evidence") await resumeStorageMigration(meteredOps,id,existing.code_revision);
      console.log(JSON.stringify({id,status:"queued",sizingHash,hotSessions:90,publicCutover:false}));return;
    }
    if(command==="sample-publications" || command==="build-cutover-evidence" || command==="complete") {
      ...
      await validateStorageValidationBootstrap(plan,{complete:bootstrap,owner:currentOwner,targetDatabaseId:target,expectedSession:expected});
      if(command==="build-cutover-evidence") {
        const built=await buildProof(owner.runId,plan.tickers,expected,plan.sourceSnapshotHash);
      ...
      const publications=await verifyStorageAcceptedPublications({env,identity:storageExecutionIdentity(existing),
        runId:owner.runId,tickers:plan.tickers,expectedSession:expected});
      if(command==="sample-publications") {
        const samples=await collectStoragePublicationGrowthSamples(env,publications,plan.capture.sourceCapture.schemaHash);
    ...
    try {
      if(command==="accept") {
        await assertOriginalCapture();
        const plan=await loadStorageValidationPlan(meteredOps,claimed.run);
        const bootstrap=await loadStorageMigrationCheckpoint(meteredOps,id,"bootstrap:complete");
        const owner=bootstrap?.payload as {runId:string;targetDatabaseId:string}|undefined;
        if(!owner || owner.targetDatabaseId!==target)throw new Error("storage-completed-bootstrap-required");
        const currentOwner=await loadStorageMigrationCheckpoint(meteredOps,id,"bootstrap:owner");
        ...
        const {proof,provenance,publications,capacity,runtime,analysis,capture,consumers}=
          await buildProof(owner.runId,plan.tickers,expected,plan.sourceSnapshotHash);
        ...
        const evidence={version:1,publications,capacity,runtimeEvidenceHash:runtime.evidenceHash,cutoverProofHash:storageProof.record.proofHash,
          storageCutoverProofId:storageProof.id,validationPlanHash:plan.planHash,
