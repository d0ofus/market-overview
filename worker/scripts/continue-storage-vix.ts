...
import { approveStorageVixContinuation } from "../src/market-storage-vix-continuation";
import { collectStorageVixCodeContract } from "./storage-vix-code-contract";

// Explicit R15 VIX identity correction. No price, provider, EOD-run, quota,
...
async function main():Promise<void> {
  ...
  await assertReviewedCheckout();const codeContract=collectStorageVixCodeContract({root,fromRevision,codeRevision});
  ...
  try {
    ...
    const result=await approveStorageVixContinuation(input);
    ...
    console.log(JSON.stringify({status:"vix-continuation-approved",migrationId:id,executionRevision:codeRevision,validationPlanHash:result.plan.planHash,
      ...
      predecessorAuditHash:result.continuation.predecessorAuditHash,featureCheckpointsPreserved:result.continuation.featureCheckpointCount,vixCheckpoint:result.continuation.vixCheckpoint,historyRevision:result.continuation.historyRevision,eodRunPreserved:true,hotSessions:90}));
...
main().catch((error:unknown)=>{
  ...
  console.error(JSON.stringify({status:"vix-continuation-not-approved",reason:measured ? "eod-d1-query-budget-estimate-exceeded"
