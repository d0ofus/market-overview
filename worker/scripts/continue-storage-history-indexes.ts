...
import { approveStorageIndexLoaderContinuation } from "../src/market-storage-history-index-continuation";
import { collectStorageIndexLoaderCodeContract } from "./storage-index-loader-code-contract";

// Explicit R14 loader-fix continuation. No price, provider, EOD-run, quota,
...
const required=(name:string)=>{const value=process.env[name]?.trim();if(!value)throw new Error("storage-index-loader-continuation-setting-missing");return value;};
const object=(value:unknown):Record<string,unknown>|null=>value!==null&&typeof value==="object"&&!Array.isArray(value)?value as Record<string,unknown>:null;
async function main():Promise<void> {
  if(process.argv.length!==2)throw new Error("storage-index-loader-continuation-command-invalid");
  ...
  if(!/^[\w.-]+\/[\w.-]+$/.test(repository)||profile.name!=="paid")throw new Error("storage-index-loader-continuation-paid-profile-required");
  const command=(exe:string,args:string[])=>{
    ...
    catch{throw new Error("storage-index-loader-continuation-control-verification-failed");}
  };
  ...
  const assertReviewedCheckout=async()=>{
    if(!/^[a-f0-9]{40}$/.test(codeRevision)||git("rev-parse","HEAD")!==codeRevision||git("branch","--show-current")!=="main"||git("status","--porcelain")
      ||object(gh(`repos/${repository}/git/ref/heads/main`).object)?.sha!==codeRevision||git("merge-base",fromRevision,codeRevision)!==fromRevision)throw new Error("storage-index-loader-continuation-clean-github-main-required");
  };
  const variables=()=>{
    const body=gh(`repos/${repository}/environments/market-eod/variables?per_page=100`);
    if(!Array.isArray(body.variables)||body.total_count!==body.variables.length)throw new Error("storage-index-loader-continuation-github-variables-incomplete");
    return new Map(body.variables.map(value=>{const row=object(value);return [String(row?.name),String(row?.value)];}));
  };
  await assertReviewedCheckout();const codeContract=collectStorageIndexLoaderCodeContract({root,fromRevision,codeRevision});
  const ids=[source,target,history,opsId];if(new Set(ids).size!==4)throw new Error("storage-index-loader-continuation-database-conflict");
  const rawOps=createEodD1Database({accountId,token,databaseId:opsId,allowedDatabaseIds:ids});
  const admission=createEodAdmission(rawOps,`index-loader-continuation:${id}`,{profile,writeCredit:200,
  ...
  try {
    ...
    if(!run||run.source_database_id!==source||run.target_database_id!==target||run.history_database_id!==history
      ...
      ||vars.get("EOD_BUDGET_PROFILE")!=="paid"||![fromRevision,codeRevision].includes(vars.get("EOD_STORAGE_EXECUTION_REVISION")??""))throw new Error("storage-index-loader-continuation-durable-identity-conflict");
    const assertRevokedRunClaimFence=async()=>{
      ...
      if(!oldRunner.includes("await assertStorageExecutionRevision(meteredOps,existing,codeRevision)")||!oldRunner.includes("githubRunId:process.env.GITHUB_RUN_ID,executionRevision:codeRevision")
        ...
        ||!oldControl.includes("AND (? IS NULL OR NOT EXISTS(SELECT 1 FROM eod_rollout_evidence WHERE id=?))")||!oldControl.includes("revocationKey,revocationKey"))throw new Error("storage-index-loader-continuation-revoked-run-fence-required");
    };
    ...
    const measurePhysical=async()=>{
      ...
      for(const databaseId of [target,history]) {
        ...
        if(!response.ok)throw new Error("storage-index-loader-continuation-capacity-read-failed");
        const body=object(await response.json()),result=object(body?.result);
        if(body?.success!==true||result?.uuid!==databaseId||typeof result.file_size!=="number"||!Number.isSafeInteger(result.file_size))throw new Error("storage-index-loader-continuation-capacity-identity-invalid");
    ...
    const result=await approveStorageIndexLoaderContinuation(input);
    command("gh",["variable","set","EOD_STORAGE_EXECUTION_REVISION","--env","market-eod","--repo",repository,"--body",codeRevision]);
    if(variables().get("EOD_STORAGE_EXECUTION_REVISION")!==codeRevision)throw new Error("storage-index-loader-continuation-github-pin-unconfirmed");
    ...
    console.log(JSON.stringify({status:"loader-continuation-approved",migrationId:id,executionRevision:codeRevision,validationPlanHash:result.plan.planHash,
      ...
      originalRecoveryHash:result.continuation.recoveryEvidenceHash,historyRevision:result.continuation.historyRevision,eodRunPreserved:true,hotSessions:90}));
...
main().catch((error:unknown)=>{
  ...
  console.error(JSON.stringify({status:"loader-continuation-not-approved",reason:measured ? "eod-d1-query-budget-estimate-exceeded"
    : /^(storage-[a-z-]+|eod-(?:account|rolling|resource|daily|budget)[a-z-]*|d1-(?:request-timeout|network-error|response-result-count-or-shape-mismatch|usage-metadata-unavailable))$/.test(message)?message:"storage-index-loader-continuation-verification-failed",
