...
import { approveStorageListingExecution, resumeStorageListingExecution } from "../src/market-storage-listing-execution";
import { collectStorageListingExecutionCodeContract } from "./storage-listing-execution-code-contract";

// Exact R18 admission of optional verified listing evidence for future daily runs. No price, provider, EOD-run, quota,
...
const required=(name:string)=>{const value=process.env[name]?.trim();if(!value)throw new Error("storage-listing-execution-setting-missing");return value;};
const object=(value:unknown):Record<string,unknown>|null=>value!==null&&typeof value==="object"&&!Array.isArray(value)?value as Record<string,unknown>:null;
async function main():Promise<void> {
  if(process.argv.length!==2)throw new Error("storage-listing-execution-command-invalid");
  ...
  if(!/^[\w.-]+\/[\w.-]+$/.test(repository)||profile.name!=="paid")throw new Error("storage-listing-execution-paid-profile-required");
  const command=(exe:string,args:string[])=>{
    ...
    catch{throw new Error("storage-listing-execution-control-verification-failed");}
  };
  ...
  const assertReviewedCheckout=async()=>{
    if(!/^[a-f0-9]{40}$/.test(codeRevision)||git("rev-parse","HEAD")!==codeRevision||git("branch","--show-current")!=="main"||git("status","--porcelain")
      ||object(gh(`repos/${repository}/git/ref/heads/main`).object)?.sha!==codeRevision||git("merge-base",fromRevision,codeRevision)!==fromRevision)throw new Error("storage-listing-execution-clean-github-main-required");
  };
  const variables=()=>{
    const body=gh(`repos/${repository}/environments/market-eod/variables?per_page=100`);
    if(!Array.isArray(body.variables)||body.total_count!==body.variables.length)throw new Error("storage-listing-execution-github-variables-incomplete");
    return new Map(body.variables.map(value=>{const row=object(value);return [String(row?.name),String(row?.value)];}));
  };
  await assertReviewedCheckout();const codeContract=collectStorageListingExecutionCodeContract({root,fromRevision,codeRevision});
  const ids=[source,target,history,opsId];if(new Set(ids).size!==4)throw new Error("storage-listing-execution-database-conflict");
  const rawOps=createEodD1Database({accountId,token,databaseId:opsId,allowedDatabaseIds:ids});
  const admission=createEodAdmission(rawOps,`listing-execution:${id}`,{profile,writeCredit:200,
  ...
  try {
    ...
    if(!run||run.source_database_id!==source||run.target_database_id!==target||run.history_database_id!==history
      ...
      ||vars.get("EOD_BUDGET_PROFILE")!=="paid"||![fromRevision,codeRevision].includes(vars.get("EOD_STORAGE_EXECUTION_REVISION")??""))throw new Error("storage-listing-execution-durable-identity-conflict");
    const assertRevokedRunClaimFence=async()=>{
      ...
      if(!oldRunner.includes("await assertStorageExecutionRevision(meteredOps,existing,codeRevision)")||!oldRunner.includes("githubRunId:process.env.GITHUB_RUN_ID,executionRevision:codeRevision")
        ...
        ||!oldControl.includes("AND (? IS NULL OR NOT EXISTS(SELECT 1 FROM eod_rollout_evidence WHERE id=?))")||!oldControl.includes("revocationKey,revocationKey"))throw new Error("storage-listing-execution-revoked-run-fence-required");
    };
    ...
    const measurePhysical=async()=>{
      ...
      for(const databaseId of [target,history]) {
        ...
        if(!response.ok)throw new Error("storage-listing-execution-capacity-read-failed");
        const body=object(await response.json()),result=object(body?.result);
        if(body?.success!==true||result?.uuid!==databaseId||typeof result.file_size!=="number"||!Number.isSafeInteger(result.file_size))throw new Error("storage-listing-execution-capacity-identity-invalid");
    ...
    const result=await approveStorageListingExecution(input);
    command("gh",["variable","set","EOD_STORAGE_EXECUTION_REVISION","--env","market-eod","--repo",repository,"--body",codeRevision]);
    if(variables().get("EOD_STORAGE_EXECUTION_REVISION")!==codeRevision)throw new Error("storage-listing-execution-github-pin-unconfirmed");
    const latest=await loadStorageMigration(ops,id);
    if(latest?.status==="awaiting-evidence"&&latest.error_code==="storage-execution-github-pin-required")await resumeStorageListingExecution(ops,id,codeRevision);
    console.log(JSON.stringify({status:"listing-execution-continuation-approved",migrationId:id,executionRevision:codeRevision,validationPlanHash:result.plan.planHash,
...
main().catch((error:unknown)=>{
  ...
  console.error(JSON.stringify({status:"listing-execution-continuation-not-approved",reason:measured ? "eod-d1-query-budget-estimate-exceeded"
    : /^(storage-[a-z-]+|eod-(?:account|rolling|resource|daily|budget)[a-z-]*|d1-(?:request-timeout|network-error|response-result-count-or-shape-mismatch|usage-metadata-unavailable))$/.test(message)?message:"storage-listing-execution-verification-failed",
