...
describe("bounded archive copy transport against real SQLite", () => {
  it("verifies eight independent blocks in four requests and replays an ambiguous committed promotion", async () => {
    ...
    try {
      ...
      const groups=Array.from({length:8},(_,index) => [bar(`TEST${index}`)]);
      ...
      expect(requestSizes).toEqual([8,16,16]);
      ...
      expect(await history.db.prepare("SELECT COUNT(*) AS count FROM market_history_blocks").first("count")).toBe(8);
      requestSizes.length=0;
      await copyStorageArchiveBlocks(wrapped,groups.map((rows) => rows.map((row) => ({...row,o:11,h:11,l:11,c:11}))));
      expect(requestSizes).toEqual([8,16,16,8]);
  ...
  it("rejects duplicate years and oversized groups before database writes", async () => {
    ...
    try {
      ...
      await expect(copyStorageArchiveBlocks(history.db,Array.from({length:9},(_,index) => [bar(`TEST${index}`)])))
