...
import { approveStorageVixContinuation,STORAGE_VIX_PREVIOUS_REVISION,storageVixContinuationKey,type StorageVixCodeContract } from "../src/market-storage-vix-continuation";
...
const revision="d".repeat(40),stamp="2026-09-11T14:00:00.000Z";
...
async function prepared() {
  ...
  for(let chunk=0;chunk<Math.ceil(tickers.length/25);chunk++) {
    const own=tickers.slice(chunk*25,chunk*25+25),metrics=own.map(ticker=>[ticker,computeEodTickerMetrics({ticker,targetSession:bridge.plan.sessionDate,
      calendarDates:calendar,bars:ticker==="VIX" ? [] : calendar.map(sessionDate=>({ticker,sessionDate,close:100,high:101,low:99,open:100,
  ...
  await storeEodPublication(env,{scope:"overview:default",sessionDate:bridge.plan.sessionDate,inputHash:"8".repeat(64),methodologyVersion:EOD_METRICS_VERSION,
    payload:{asOfDate:bridge.plan.sessionDate,sections:[{groups:[{rows:[{ticker:"VIX",price:null,change1d:null},{ticker:"SPY",price:100,change1d:0}]}]}]},promote:true,revisions:[]});
...
describe("VIX identity correction preserves the actual private bootstrap",()=>{
  it("rejects writer/checkpoint races, preserves 253 encoded checkpoints and Overview, and resumes the real amended pipeline",{timeout:240_000},async()=>{
    ...
    const {planHash:oldHash,...oldFields}=result.plan,successorFields={...oldFields,sessionDate:"2026-09-11",predecessorPlanHash:oldHash,
    ...
    expect(await loadStorageHistoryIndexAmendment(ops.db,actualRun,successor)).toEqual(f.amendment);
    await expect(loadStorageHistoryIndexAmendment(ops.db,actualRun,{...successor,tickers:["WRONG"]})).rejects.toThrow("lineage-mismatch");
    await resumeStorageMigration(ops.db,identity.id,identity.codeRevision);
    const claimed=await claimStorageMigration(ops.db,identity.id,{executionRevision:revision});expect(claimed).not.toBeNull();
    ...
    await expect(runStoragePipeline({source:source.db,target:target.db,history:history.db,ops:ops.db,run:claimed!.run,leaseToken:claimed!.leaseToken,
      installSourceFence:install,installTargetFence:install,installHistoryFence:install,bootstrapEnv:f.env,bootstrapFailureDb:ops.db})).rejects.toThrow("vix-real-price-boundary");
