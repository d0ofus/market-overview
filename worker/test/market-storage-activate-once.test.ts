...
async function protocol() {
  ...
  const deps: StorageActivationDependencies = {
    ...
    verifyGitHub: vi.fn(async () => ({ marketDatabaseId, mode })),
    setGitHubVariable: vi.fn(async (name, value) => { events.push(name); if (name === "EOD_MARKET_DATABASE_ID") marketDatabaseId = value; else mode = value as "active"; }),
  ...
  return { deps, events, state, setState: (value: StorageActivationState) => { state = value; },
    ...
    setGitHub: (id: string, value: "shadow" | "active") => { marketDatabaseId = id; mode = value; } };
}
...
describe("recoverable activation stages", () => {
  it("verifies publications, authenticates, sets target then mode, deploys and verifies before completing", async () => {
    ...
    expect(f.events).toEqual(["publications", "authenticate", "EOD_MARKET_DATABASE_ID", "EOD_RUNNER_MODE", "publications", "deploy", "verify-target", "complete", "verify-target"]);
  });
  it.each(["EOD_MARKET_DATABASE_ID", "EOD_RUNNER_MODE"] as const)("resumes after %s persisted but response was lost", async (failure) => {
    ...
    expect(f.deps.setGitHubVariable).toHaveBeenCalledTimes(2);
  });
